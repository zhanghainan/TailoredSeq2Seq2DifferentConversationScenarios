...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    with tf.variable_scope(self.scope_name):
        ...
        self.learning_rate_set_op = self.learning_rate.assign(self.learning_rate * 0.1)
        ...
        def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
          return rl_seq2seq.embedding_attention_seq2seq(
              ...
              dtype=dtype,beam_search=beam_search, beam_size=beam_size)
        ...
        #if forward_only:
        if beam_search:
          self.outputs, self.beam_path, self.beam_symbol = rl_seq2seq.decode_model_with_buckets(self.encoder_inputs, self.decoder_inputs, targets,self.target_weights, buckets, lambda x, y: seq2seq_f(x, y, True),softmax_loss_function=softmax_loss_function,per_example_loss=True)
        else:
          self.outputs, self.losses, self.encoder_state = rl_seq2seq.model_with_buckets(
              ...
              softmax_loss_function=softmax_loss_function,per_example_loss=True,FLAGtrain=tf.select(self.force_dec_input,True,False))
        ...
        if not forward_only:
          ...
          for b in xrange(len(buckets)):
            self.adjusted_losses =self.losses[b]#tf.reduce_sum( tf.matmul(tf.reshape(self.losses[b],[1,-1]), tf.reshape(self.advantage[b],[-1,1])))
        ...
        self.saver = tf.train.Saver(all_variables)#,max_to_keep=200)
        from tensorflow.python.ops import variable_scope
        for var in tf.trainable_variables():
          print(var.name)
          if var.name == 'seq2seq/embedding_attention_seq2seq/embedding:0':
            self.embedding = var
  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
           bucket_id, forward_only=True, force_dec_input=False, advantage=None, debug=False,beam_search=False):
    ...
    if not forward_only: # normal training
      ...
      if beam_search:
        ...
        output_feed.append(self.embedding)
      else:
    ...
    if not forward_only:
      ...
      if beam_search:
        return outputs[0], outputs[1], outputs[2], outputs[3:],  # No gradient norm, loss, outputs.
      else:
        return None, outputs[0], outputs[1:]  # encoder_state, loss, outputs.
...
# log(P(b|a)), the conditional likelyhood
  ...
  def get_batch(self, train_data, bucket_id, type=0, fixed_set=None, weibo_set=None, qa_set=None, num=0):
    ...
    for batch_i in xrange(self.batch_size):
      ...
      if type == 0:
        if fixed_train_num > batch_i:
          ...
          encoder_input, decoder_input = train_data[bucket_id][(num+batch_i)%leng]#random.choice(train_data[bucket_id])
