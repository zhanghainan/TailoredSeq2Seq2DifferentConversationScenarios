...
def _extract_beam_search(embedding, beam_size, num_symbols, embedding_size,  output_projection=None,
  ...
  def loop_function(prev, i, log_beam_probs, beam_path, beam_symbols):
    ...
    if i > 1:
        ...
        #print(log_beam_probs[-1].get_shape())
    #print(probs.get_shape())
  ...
  with variable_scope.variable_scope(scope or "attention_decoder"):
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      outputs.append(tf.argmax(nn_ops.xw_plus_b(
          output, output_projection[0], output_projection[1]), dimension=1))
  if beam_size==1:
    return outputs, state, tf.reshape(beam_path,[-1,beam_size]),tf.reshape(beam_symbols,[-1,beam_size])
  else:
    return outputs, state, tf.reshape(tf.concat(0, beam_path),[-1,beam_size]), tf.reshape(tf.concat(0, beam_symbols),[-1,beam_size])
  ...
  with variable_scope.variable_scope(
    ...
    if beam_search:
      return beam_attention_decoder(
            ...
            initial_state_attention=initial_state_attention, output_projection=output_projection, beam_size=beam_size,scope=scope)
    else:
  ...
  with variable_scope.variable_scope(
    ...
    if isinstance(feed_previous, bool):
      ...
      return embedding_attention_decoder(
          ...
          feed_previous=feed_previous,
          initial_state_attention=initial_state_attention,
          scope=scope, beam_search=beam_search, beam_size=beam_size)
...
def sequence_loss_by_example(logits, targets, weights,
                             ...
                             softmax_loss_function=None, name=None,FLAGtrain=False):
  ...
  with ops.name_scope(name, "sequence_loss_by_example",
    ...
    if FLAGtrain==False:
      return log_perps
    log_perps_reshape = tf.reshape(log_perps,[20,10])
    log_perps_top,indexx = tf.nn.top_k(log_perps_reshape,1)
    log_perps_reshape2=tf.reshape(log_perps_top,[-1])
  return log_perps_reshape2


def sequence_loss(logits, targets, weights,
                  average_across_timesteps=True, average_across_batch=True,
                  softmax_loss_function=None,FLAGtrain=False, name=None):
  ...
  with ops.name_scope(name, "sequence_loss", logits + targets + weights):
    cost = math_ops.reduce_sum(sequence_loss_by_example(
        ...
        softmax_loss_function=softmax_loss_function,FLAGtrain=FLAGtrain))
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
                       ...
                       per_example_loss=False,FLAGtrain=False, name=None):
...
def decode_model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
  ...
  with ops.op_scope(all_inputs, name, "model_with_buckets"):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                                         reuse=True if j > 0 else None):
        bucket_outputs, _, beam_path, beam_symbol = seq2seq(encoder_inputs[:bucket[0]],decoder_inputs[:bucket[1]])
        outputs.append(bucket_outputs)
